...
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz,int va)
{
  char *mem;
  uint a;
  struct proc *p = proc;
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    memset(mem, 0, PGSIZE);
 //  if((oldsz == 0&&a==0)&&*proc->name=='s')
 if(va && (oldsz == 0 && a == 0))
{
    mappages(pgdir, (char*)a, PGSIZE, PADDR(mem), 6);
   //cprintf("%s",proc->name);
}
else
...
for(i =p->sz2; i < USERTOP; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: pte should exist");
    if((!(*pte & PTE_P)))
      panic("copyuvm: page not present");
